...
def train_quine(network, 
    ...
    if not success:
        ...
        network, training_time, success = train_quine(network, k*10, shift, max_iterations, tolerance)
        return network, training_time, success
...
def run_experiments():
    ...
    for use_ortho in [False, True]:
        for config in configs:
            ...
            # Train network
            network, training_time, success = train_quine(
                network, 
                k=config['k'], 
                shift=config['shift']
            )
